...
There are other ways to construct causal graphs too. Often, we can derive it, when systems already convey this kind of information.
For example, in a distributed system of microservices, we can use request tracing frameworks to reconstruct the graph of
...
Note that, depending on the causal task, this graph doesn’t need to be completely directed. For instance, for
...
* For many other tasks, we will also have to assign so-called causal mechanisms to each node as we’ll show in the
  following section.

To diagnose and potentially reject a causal graph, check out :doc:`diagnosing_a_causal_model/index`.
